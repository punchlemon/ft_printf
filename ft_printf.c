...
/*   Updated: 2024/05/07 19:55:47by retanaka         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	get_chr_base(int num, int case_flag)
{
	if (num < 10)
		return ((unsigned char)num + '0');
	if (case_flag == 1)
		return ((unsigned char)num + 'A');
	else
		return ((unsigned char)num + 'a');
}
...
void	convert_base_num(char base_chr, int *base, int *sign, int *case_flag)
{
	*base = 10;
	*case_flag = 0;
	*sign = 0;
	if (base_chr == 'd' || base_chr == 'i')
		*sign = 1;
	else if (base_chr == 'o')
		*base = 8;
...
char	*convert_num_str(int num, char base_chr)
{
	...
	convert_base_num(base_chr, &base, &sign, &case_flag);
	result = NULL;
	if (sign == 0 || num > 0)
		unsigned_num = (unsigned int)(num);
	if ((long)num == -2147483648 && sign == 1)
	...
	while (unsigned_num >= 10)
		concat_chr(&result, convert_num_chr(&unsigned_num, base, case_flag));
	concat_chr(&result, convert_num_chr(&unsigned_num, base, case_flag));
	return (result);
}
